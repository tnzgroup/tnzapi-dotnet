...
public class EmailControllerTests : DemoApiTestBase
{
    ...
    public async Task Abort_SendsPatchToAbortEndpoint()
    {
        ...
        var response = await Client.PatchAsync("/api/email/abc-123/abort", null);

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("https://api.tnz.co.nz/api/v3.00/email/abc-123/abort", handler.LastRequest!.RequestUri!.ToString());
    }
    ...
    public async Task Reschedule_SendsPatchWithSendTime()
    {
        ...
        var response = await Client.PatchAsJsonAsync("/api/email/abc-123/reschedule", new { SendTime = "2026-08-01 09:00:00" });

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("https://api.tnz.co.nz/api/v3.00/email/abc-123/reschedule", handler.LastRequest!.RequestUri!.ToString());
    ...
    public async Task Reschedule_WithInvalidSendTime_ReturnsBadRequestWithoutCallingSdk()
    {
        ...
        var response = await Client.PatchAsJsonAsync("/api/email/abc-123/reschedule", new { SendTime = "not-a-date" });
    ...
    public async Task Resubmit_SendsPatchWithSendTime()
    {
        ...
        var response = await Client.PatchAsJsonAsync("/api/email/abc-123/resubmit", new { SendTime = "2026-08-01 09:00:00" });

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("https://api.tnz.co.nz/api/v3.00/email/abc-123/resubmit", handler.LastRequest!.RequestUri!.ToString());
        Assert.Contains("SendTime", handler.LastRequestBody);
    }
    ...
    public async Task Resubmit_WithInvalidSendTime_ReturnsBadRequestWithoutCallingSdk()
    {
        ...
        var response = await Client.PatchAsJsonAsync("/api/email/abc-123/resubmit", new { SendTime = "not-a-date" });
