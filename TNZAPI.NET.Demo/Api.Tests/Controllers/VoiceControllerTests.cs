...
public class VoiceControllerTests : DemoApiTestBase
{
    public VoiceControllerTests(DemoApiFactory factory) : base(factory)
    {
    }
    ...
    public async Task Send_OnSuccess_Returns200WithFullResult()
    {
        ...
        var response = await Client.PostAsJsonAsync("/api/voice/send", new { ToNumber = "+6421000001", MessageToPeople = "base64audio==" });
    ...
    public async Task Send_WithUnparseableSendTime_ReturnsBadRequestWithoutCallingSdk()
    {
        ...
        var response = await Client.PostAsJsonAsync("/api/voice/send", new { ToNumber = "+6421000001", MessageToPeople = "base64audio==", SendTime = "not-a-date" });
    ...
    public async Task Status_GetsCorrectSdkUrl()
    {
        ...
        var response = await Client.GetAsync("/api/voice/status/abc-123");
