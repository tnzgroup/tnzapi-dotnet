...
{
    public record RcsAttachmentRequest(string FileName, string FileContent);
    public record RcsRescheduleRequest(string SendTime);
    ...
    // identical to SMSModel — same field set, only the FallbackMode enum differs (RCSFallbackMode:
    // None/SMS/Voice/WhatsApp vs SMS's SMSFallbackMode: None/Voice/RCS/WhatsApp).
    public record SendRcsRequest(
        string ToNumber,
        string Message,
        string? Reference,
        string? TemplateId,
        ...
        string? SendMode,
        ICollection<RcsAttachmentRequest>? Attachments
    );
    ...
    public class RcsController : ControllerBase
    {
        ...
        public RcsController(TNZApiClient client)
        {
            _client = client;
        }

        [HttpPost("send")]
        public async Task<IActionResult> Send([FromBody] SendRcsRequest request)
        {
            try
            {
                var entity = new RCSModel
                {
                ...
                if (request.FallbackMode is { Count: > 0 })
                {
                    entity.FallbackMode = request.FallbackMode
                        .Where(m => Enum.TryParse<Enums.RCSFallbackMode>(m, out _))
                        .Select(m => Enum.Parse<Enums.RCSFallbackMode>(m))
                        .ToList();
                }
                ...
                var result = await _client.Messaging.RCS.SendMessageAsync(entity);
        ...
        public async Task<IActionResult> Status(string id)
        {
            try
            {
                var result = await _client.Messaging.RCS.StatusAsync(new MessageID(id));
        ...
        public async Task<IActionResult> Abort(string id)
        {
            try
            {
                var result = await _client.Actions.RCS.AbortAsync(new MessageID(id));
        ...
        public async Task<IActionResult> Reschedule(string id, [FromBody] RcsRescheduleRequest request)
        {
            try
            {
                if (!DateTime.TryParse(request.SendTime, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out var sendTime))
                ...
                var result = await _client.Actions.RCS.RescheduleAsync(new MessageID(id), sendTime);
        ...
        // timePeriod when supplied — RCSApi exposes this as two distinct overloads, so we pick
            ...
            [FromQuery] string? dateTo = null)
        {
            try
            {
                ...
                var result = parsedDateFrom is not null
                    ? await _client.Messaging.RCS.ReceivedAsync(parsedDateFrom.Value, parsedDateTo, recordsPerPage, page)
                    : await _client.Messaging.RCS.ReceivedAsync(timePeriod, recordsPerPage, page);
