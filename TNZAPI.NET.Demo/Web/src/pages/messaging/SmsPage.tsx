...
import type { SendSmsRequest, MessageApiResult, MessageStatusApiResult } from '@/lib/api-client'

const FALLBACK_MODES = ['Voice', 'RCS', 'WhatsApp']
...
export default function SmsPage() {
  ...
  async function handleSend(e: FormEvent) {
    ...
    const { status, data } = await apiRequest<MessageApiResult>('/api/sms/send', { method: 'POST', body })
  ...
  async function handleCheckStatus(e: FormEvent) {
    ...
    const { status, data } = await apiRequest<MessageStatusApiResult>(
      `/api/sms/status/${encodeURIComponent(id)}`,
    )
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">client.Messaging.SMS.SendMessageAsync</p>
      </div>
