...
public class EmailApiActionsTests : IDisposable
{
    public void Dispose()
    {
    ...
    public async Task RescheduleAsync_PatchesCorrectUrlWithSendTime()
    {
        ...
        await email.RescheduleAsync(new MessageID("abc-123"), sendTime);
    ...
    public async Task AbortAsync_PatchesCorrectUrlWithNoBody()
    {
        ...
        await email.AbortAsync(new MessageID("abc-123"));
    ...
    public async Task ResubmitAsync_PatchesCorrectUrlWithSendTime()
    {
        ...
        var result = await email.ResubmitAsync(new MessageID("abc-123"), sendTime);
    ...
    public async Task RescheduleAsync_WithEmptyMessageID_FailsClientSideWithoutNetworkCall()
    {
        ...
        var result = await email.RescheduleAsync(new MessageID(""), DateTime.Now);
