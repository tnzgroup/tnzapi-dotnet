...
public class EmailApiStatusTests : IDisposable
{
    public void Dispose()
    {
    ...
    public async Task StatusAsync_GetsCorrectUrl()
    {
        ...
        await email.StatusAsync(new MessageID("abc-123"));

        Assert.Equal("https://api.tnz.co.nz/api/v3.00/email/abc-123", fakeHandler.LastRequest!.RequestUri!.ToString());
    ...
    {
        var body = "{\"MessageID\":\"abc-123\",\"JobStatus\":\"Completed\",\"Recipients\":[{\"Type\":\"Email\",\"Destination\":\"john.doe@example.com\",\"Status\":\"Success\",\"Result\":\"250 OK\"}]}";
        var fakeHandler = new FakeHttpMessageHandler(HttpStatusCode.OK, body);
        HttpRequest.MessageHandler = fakeHandler;
        var email = new EmailApi(new TNZApiUser { AuthToken = "test-token" });

        var result = await email.StatusAsync(new MessageID("abc-123"));
        ...
        Assert.Equal(Enums.RecipientChannelType.Email, result.Recipients[0].Type);
        Assert.Equal("250 OK", result.Recipients[0].Result);
    }
    ...
    public async Task StatusAsync_ParsesNumericPrice()
    {
        ...
        var body = "{\"MessageID\":\"abc-123\",\"JobStatus\":\"Completed\",\"Price\":0.1,\"Recipients\":[{\"Type\":\"Email\",\"Destination\":\"john.doe@example.com\",\"Status\":\"Success\",\"Result\":\"250 OK\",\"Price\":0.05}]}";
        var fakeHandler = new FakeHttpMessageHandler(HttpStatusCode.OK, body);
        HttpRequest.MessageHandler = fakeHandler;
        var email = new EmailApi(new TNZApiUser { AuthToken = "test-token" });
    ...
    public async Task StatusAsync_WithEmptyMessageID_FailsClientSideWithoutNetworkCall()
    {
        ...
        var result = await email.StatusAsync(new MessageID(""));
    ...
    public void Status_Sync_DelegatesToAsync()
    {
        ...
        var result = email.Status(new MessageID("abc-123"));
