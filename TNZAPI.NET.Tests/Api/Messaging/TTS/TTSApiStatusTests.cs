...
public class TTSApiStatusTests : IDisposable
{
    public void Dispose()
    {
    ...
    public async Task StatusAsync_GetsCorrectUrl()
    {
        ...
        await tts.StatusAsync(new MessageID("abc-123"));

        Assert.Equal("https://api.tnz.co.nz/api/v3.00/tts/abc-123", fakeHandler.LastRequest!.RequestUri!.ToString());
    ...
    {
        var body = "{\"MessageID\":\"abc-123\",\"JobStatus\":\"Completed\",\"Recipients\":[{\"Type\":\"Voice\",\"Destination\":\"+6421000001\",\"Status\":\"Success\",\"Result\":\"SentOK\"}]}";
        var fakeHandler = new FakeHttpMessageHandler(HttpStatusCode.OK, body);
        HttpRequest.MessageHandler = fakeHandler;
        var tts = new TTSApi(new TNZApiUser { AuthToken = "test-token" });

        var result = await tts.StatusAsync(new MessageID("abc-123"));
        ...
        Assert.Equal(Enums.RecipientChannelType.Voice, result.Recipients[0].Type);
        Assert.Equal("SentOK", result.Recipients[0].Result);
    }

    [Fact]
    public async Task StatusAsync_UnrecognizedRecipientTypeFallsBackToUnknown()
    {
        // Confirmed via the live API response: TTS and Voice calls both come back as "Voice" on the
        // wire (not a channel-specific value) — the StatusAsync_ParsesJobStatusAndRecipients test
        // above already covers that case. This covers a wire value the SDK doesn't model (e.g.
        // "Postal") — it should degrade to Unknown rather than throw, since TNZ's wire vocabulary
        // here has already proven unpredictable once.
        var body = "{\"MessageID\":\"abc-123\",\"JobStatus\":\"Completed\",\"Recipients\":[{\"Type\":\"Postal\",\"Destination\":\"+6421000001\",\"Status\":\"Success\",\"Result\":\"SentOK\"}]}";
        var fakeHandler = new FakeHttpMessageHandler(HttpStatusCode.OK, body);
        HttpRequest.MessageHandler = fakeHandler;
        var tts = new TTSApi(new TNZApiUser { AuthToken = "test-token" });
        ...
        Assert.Equal(Enums.ResultCode.Success, result.Result);
        Assert.Single(result.Recipients);
        Assert.Equal(Enums.RecipientChannelType.Unknown, result.Recipients[0].Type);
    }
    ...
    public async Task StatusAsync_ParsesNumericPrice()
    {
        ...
        var body = "{\"MessageID\":\"abc-123\",\"JobStatus\":\"Completed\",\"Price\":0.1,\"Recipients\":[{\"Type\":\"Voice\",\"Destination\":\"+6421000001\",\"Status\":\"Success\",\"Result\":\"SentOK\",\"Price\":0.05}]}";
        var fakeHandler = new FakeHttpMessageHandler(HttpStatusCode.OK, body);
        HttpRequest.MessageHandler = fakeHandler;
        var tts = new TTSApi(new TNZApiUser { AuthToken = "test-token" });
    ...
    public async Task StatusAsync_WithEmptyMessageID_FailsClientSideWithoutNetworkCall()
    {
        ...
        var result = await tts.StatusAsync(new MessageID(""));
    ...
    public void Status_Sync_DelegatesToAsync()
    {
        ...
        var result = tts.Status(new MessageID("abc-123"));
