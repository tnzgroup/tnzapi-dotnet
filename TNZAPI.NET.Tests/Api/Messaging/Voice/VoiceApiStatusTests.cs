...
public class VoiceApiStatusTests : IDisposable
{
    public void Dispose()
    {
    ...
    public async Task StatusAsync_GetsCorrectUrl()
    {
        ...
        await voice.StatusAsync(new MessageID("abc-123"));

        Assert.Equal("https://api.tnz.co.nz/api/v3.00/voice/abc-123", fakeHandler.LastRequest!.RequestUri!.ToString());
    ...
    public async Task StatusAsync_ParsesJobStatusAndRecipients()
    {
        ...
        var result = await voice.StatusAsync(new MessageID("abc-123"));
    ...
    public async Task StatusAsync_WithEmptyMessageID_FailsClientSideWithoutNetworkCall()
    {
        ...
        var result = await voice.StatusAsync(new MessageID(""));
    ...
    public void Status_Sync_DelegatesToAsync()
    {
        ...
        var result = voice.Status(new MessageID("abc-123"));
