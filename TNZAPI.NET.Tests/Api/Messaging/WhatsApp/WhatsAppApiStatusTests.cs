...
public class WhatsAppApiStatusTests : IDisposable
{
    public void Dispose()
    {
    ...
    public async Task StatusAsync_GetsCorrectUrl()
    {
        ...
        await whatsApp.StatusAsync(new MessageID("abc-123"));

        Assert.Equal("https://api.tnz.co.nz/api/v3.00/whatsapp/abc-123", fakeHandler.LastRequest!.RequestUri!.ToString());
    ...
    {
        var body = "{\"MessageID\":\"abc-123\",\"JobStatus\":\"Completed\",\"Recipients\":[{\"Type\":\"WhatsApp\",\"Destination\":\"+64211234567\",\"Status\":\"Success\",\"Result\":\"Delivered\",\"SMSReplies\":[{\"From\":\"+64211234567\",\"MessageText\":\"Yes please\"}]}]}";
        var fakeHandler = new FakeHttpMessageHandler(HttpStatusCode.OK, body);
        HttpRequest.MessageHandler = fakeHandler;
        var whatsApp = new WhatsAppApi(new TNZApiUser { AuthToken = "test-token" });

        var result = await whatsApp.StatusAsync(new MessageID("abc-123"));
        ...
        Assert.Equal(Enums.RecipientChannelType.WhatsApp, result.Recipients[0].Type);
    ...
    public async Task StatusAsync_ParsesNumericPrice()
    {
        ...
        var body = "{\"MessageID\":\"abc-123\",\"JobStatus\":\"Completed\",\"Price\":0.1,\"Recipients\":[{\"Type\":\"WhatsApp\",\"Destination\":\"+64211234567\",\"Status\":\"Success\",\"Result\":\"Delivered\",\"Price\":0.05}]}";
        var fakeHandler = new FakeHttpMessageHandler(HttpStatusCode.OK, body);
        HttpRequest.MessageHandler = fakeHandler;
        var whatsApp = new WhatsAppApi(new TNZApiUser { AuthToken = "test-token" });
    ...
    public async Task StatusAsync_WithEmptyMessageID_FailsClientSideWithoutNetworkCall()
    {
        ...
        var result = await whatsApp.StatusAsync(new MessageID(""));
    ...
    public void Status_Sync_DelegatesToAsync()
    {
        ...
        var result = whatsApp.Status(new MessageID("abc-123"));
